...
def changeVaraibles(config, ckpt_file_path, log_file_path, key_variable_value, key_variable='alpha'):
    config_keys = config['train']['criterion']['org_term']['criterion']['params'].keys()
    if key_variable in config_keys:
        ckpt_file_path = ckpt_file_path.split(".pt")[0]
        log_file_path = log_file_path.split(".log")[0]
        if key_variable_value is None: 
            key_variable_value = config['train']['criterion']['org_term']['criterion']['params'][key_variable]
        else:
            config['train']['criterion']['org_term']['criterion']['params'][key_variable] = key_variable_value
        ckpt_file_path += "_" + key_variable + "_" + str(key_variable_value)
        log_file_path +=  "_" + key_variable + "_" + str(key_variable_value)
        ckpt_file_path += ".pt"
        log_file_path  += ".log"
    return config, ckpt_file_path, log_file_path, key_variable_value
...
def get_argparser():
    ...
    parser.add_argument('--temp', type=float,  default=None, help='Tempurature of KD')
    parser.add_argument('--index', type=float,  default=None, help='index of the root BH')
...
def main(args):
    ...
    if not args.test_only:
        config, ckpt_file_path, log_file_path, args.alpha =  \
                changeVaraibles(config, ckpt_file_path, log_file_path, args.alpha, 'alpha') # key that is found in the yaml file
        config, ckpt_file_path, log_file_path, args.alpha =  \
                changeVaraibles(config, ckpt_file_path, log_file_path, args.index, 'index')
        config, ckpt_file_path, log_file_path, args.temp =  \
                changeVaraibles(config, ckpt_file_path, log_file_path, args.temp, 'temperature')
