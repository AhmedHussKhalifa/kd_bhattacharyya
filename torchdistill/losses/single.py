...
class KDLoss(nn.KLDivLoss):
    ...
    def forward(self, student_output, teacher_output, targets=None, *args, **kwargs):
        ...
        KD = self.alpha * hard_loss + self.beta * (self.temperature ** 2) * soft_loss
        return [KD, hard_loss.item(), soft_loss.item()]


@register_org_loss
class BHLoss(nn.KLDivLoss):
    """
    "Bhattacharyya Knowledge Distillation"
    """
    def __init__(self, temperature, index=0.5, alpha=None, beta=None, reduction='batchmean', **kwargs):
        super().__init__(reduction=reduction)
    ...
    def forward(self, student_output, teacher_output, targets=None, *args, **kwargs):
        # soft_loss_1 = torch.sum(torch.sqrt(torch.mul(torch.softmax(student_output / self.temperature, dim=1),\
        #                         torch.softmax(teacher_output / self.temperature, dim=1))), dim=1)
        soft_loss = torch.sum(torch.pow(torch.mul(torch.softmax(student_output / self.temperature, dim=1),\
                                torch.softmax(teacher_output / self.temperature, dim=1)), self.index), dim=1)
        soft_loss = torch.mean(soft_loss)
        if self.alpha is None or self.alpha == 0 or targets is None:
            return soft_loss
        hard_loss = self.cross_entropy_loss(student_output, targets)
        # soft_loss = - torch.log2(soft_loss)
        
        KD = self.alpha * hard_loss - self.beta * soft_loss
        return [KD, hard_loss.item(), soft_loss.item()]
